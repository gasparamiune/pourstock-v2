...
import { Plus, Send, Pencil, Trash2, Sparkles } from 'lucide-react';
...
export function ReleaseManager() {
  ...
  const resetForm = () => {
    setVersion(''); setTitle(''); setSummary(''); setContent('');
    setRawNotes(''); setSeverity('info'); setAudienceType('all');
    setIsMandatory(false); setIsSilent(false); setEditingId(null); setShowForm(false);
  };
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingId) {
        ...
        const { error } = await supabase.from('release_announcements').insert(payload as any);
        if (error) throw error;
      }
    },
    onSuccess: () => {
  ...
  const publishMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('release_announcements')
        .update({ is_published: true, published_at: new Date().toISOString() } as any).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage what users see after deployments</p>
        </div>
      ...
      <Dialog open={showForm} onOpenChange={(v) => { if (!v) resetForm(); }}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingId ? 'Edit Release' : 'New Release Announcement'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label>Version</Label>
                <Input placeholder="2026-03-10-v1" value={version} onChange={(e) => setVersion(e.target.value)} />
              </div>
              <div className="space-y-2">
            ...
            <div className="space-y-2">
              <Label>Title</Label>
              <Input placeholder="What's new in PourStock" value={title} onChange={(e) => setTitle(e.target.value)} />
            </div>
            <div className="space-y-2">
              <Label>Summary (optional subtitle)</Label>
              <Input placeholder="A quick look at the latest improvements" value={summary} onChange={(e) => setSummary(e.target.value)} />
            </div>
            <Separator />
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              <Label>User-Facing Content (one item per line, max 7)</Label>
              <Textarea placeholder={"🎯 New check-in flow\n📄 Fixed PDF import\n🖨️ Improved print layout"} value={content} onChange={(e) => setContent(e.target.value)} rows={6} />
            </div>
            <Separator />
            <div className="space-y-2">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button onClick={() => saveMutation.mutate()} disabled={!version || !title || !content || saveMutation.isPending}>
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {!r.is_published && (
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-primary" onClick={() => publishMutation.mutate(r.id)}>
