...
export function KitchenTicket({ order, onMarkReady, onVoid, isNew = false }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      // Fetch ALL order lines for this table (including unfired)
          ...
          {isDailyMenu && !order.items.some(i => i.source === 'alacarte') ? (
            Object.values(groupedItems).map((item, i) => (
              <div key={i}>
                ...
                {item.notes && (
                  <div className="text-[11px] text-gray-600 ml-3">↳ ×1 {item.notes}</div>
                )}
              </div>
            ))
          ) : (
        ...
        {showFull && (
          <div className="text-[11px] text-gray-700 pt-1 space-y-1">
            ...
            {COURSE_ORDER.map(course => {
              // Merge: order lines (expected) + ticket status
              ...
              // If no lines and no tickets, still show course header with 0
                ...
                <div key={course} className={cn(items.length === 0 && 'opacity-30')}>
                  <div className="flex items-center justify-between">
                    ...
                    <span className={cn(
                      ...
                      {status === 'served' ? '✓ SERVED' : status === 'ready' ? '● READY' : status === 'pending' ? '… FIRED' : '— PENDING'}
                    </span>
                  </div>
